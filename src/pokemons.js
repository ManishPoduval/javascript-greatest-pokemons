...
function shortestPokemon(pokemons) {
  ...
  const shortest = pokemons.reduce(
    (currentSortest, pokemon) =>
      pokemon.height < currentSortest.height ? pokemon : currentSortest,
    pokemons[0]
  );
...
function candyAverage(pokemons) {
  ...
  const totalCount = pokemons.reduce(
    (sum, pokemon) => (pokemon.candy_count ? sum + pokemon.candy_count : sum),
    0
  );
...
function getGroundPokeImg(pokemons) {
  ...
  const groundPokemonImages = groundPokemons
    .slice(0, 10)
